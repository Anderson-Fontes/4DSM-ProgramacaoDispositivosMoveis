...
const professorController = {
    // Lista as matérias (Para professor lista as dele, para diretor/master lista todas)
    async minhasDisciplinas(req, res) {
        try {
            let query = '';
            let params = [];

            if (req.usuario.perfil === 'diretor' || req.usuario.perfil === 'master') {
    ...
    // Busca as atividades que já foram criadas numa disciplina
    async buscarAtividades(req, res) {
        const { disciplina_id } = req.params;
        try {
            const result = await db.query("SELECT id, nome FROM atividades WHERE disciplina_id = $1 ORDER BY id DESC", [disciplina_id]);
            res.json(result.rows);
        } catch (e) {
            res.status(500).json({ erro: e.message });
        }
    ...
    // Busca todos os alunos da turma juntamente com as notas de uma atividade específica
    async buscarNotasDaAtividade(req, res) {
        const { disciplina_id, atividade_id } = req.params;
        try {
            const query = `
                SELECT al.id, al.nome, al.matricula, na.nota 
                FROM alunos al
                JOIN disciplinas d ON al.curso = d.curso
                LEFT JOIN notas_atividades na ON al.id = na.aluno_id AND na.atividade_id = $2
                WHERE d.id = $1
                ORDER BY al.nome
            `;
            const result = await db.query(query, [disciplina_id, atividade_id]);
            res.json(result.rows);
        } catch (e) {
            res.status(500).json({ erro: e.message });
        }
